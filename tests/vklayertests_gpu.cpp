...
TEST_F(VkGpuAssistedLayerTest, GpuBufferDeviceAddressOOB) {
    ...
    if (mesh_shader_supported) { m_device_extension_names.push_back(VK_NV_MESH_SHADER_EXTENSION_NAME); }
    ...
    if (mesh_shader_supported) {
        ...
        pushConstants[1] = 5;
        vk::CmdPushConstants(m_commandBuffer->handle(), mesh_pipeline_layout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(pushConstants), pushConstants);
        vk::CmdSetViewport(m_commandBuffer->handle(), 0, 1, &viewport);
        vk::CmdSetScissor(m_commandBuffer->handle(), 0, 1, &scissors);
        vk::CmdDraw(m_commandBuffer->handle(), 3, 1, 0, 0);
        vk::CmdEndRenderPass(m_commandBuffer->handle());
...
TEST_F(VkDebugPrintfTest, MeshTaskShadersPrintf) {
    ...
    InitDebugPrintfFramework();
    std::vector<const char *> required_device_extensions = {VK_NV_MESH_SHADER_EXTENSION_NAME, VK_KHR_SHADER_NON_SEMANTIC_INFO_EXTENSION_NAME};
